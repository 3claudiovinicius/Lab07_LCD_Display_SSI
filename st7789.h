...
/* ----------------------- Define by compiler ---------------------- */
	...
	#define ST7789_GPIO_WritePin(gpiox , gpio_pin , pin_state)             GPIOPinWrite(gpiox, gpio_pin, (pin_state)?gpio_pin:0)
	#endif
	
	#ifndef ST7789_GPIO_PIN_SET
		#define ST7789_GPIO_PIN_SET                                          0xFF
	#endif
	
	#ifndef ST7789_GPIO_PIN_RESET
		#define ST7789_GPIO_PIN_RESET                                        0x00
	#endif
	
  #define ST7789_Delay(t)                                                SysCtlDelay((120000000 / 3 / 1000) * (t))
	
	#ifndef ST7789_SPI_TransmitReceive
    #define ST7789_SPI_Transmit(pTxData, Size, Timeout)              \
        do {                                                \
